...
export function EditTaskModal({ task, projects, onClose, onSubmit }: EditTaskModalProps) {
  ...
  useEffect(() => {
    // Extract screenshots from description if they exist
    const imageRegex = /!\[Screenshot\]\((data:image\/[^)]+)\)/g;
    const matches = [...task.description.matchAll(imageRegex)];
    const extractedScreenshots = matches.map(match => match[1]);
    setScreenshots(extractedScreenshots);

    // Remove screenshots from description for editing
    const cleanDescription = task.description.replace(/!\[Screenshot\]\((data:image\/[^)]+)\)/g, '').trim();
    ...
    setFormData({
      ...
      created_on: task.created_on ? new Date(task.created_on).toISOString().split('T')[0] : '',
      completed_on: task.completed_on ? new Date(task.completed_on).toISOString().split('T')[0] : ''
    });
  }, [task]);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex space-x-3 pt-4">
            <button 
              ...
              disabled={uploading}
            >
              {uploading ? 'Processing...' : 'Update Task'}
            </button>
