/*
  ...
  1. New Tables
    - `receivables` - Track billable amounts from completed tasks
    - `revenue_records` - Track payments received against receivables
    - `invoices` - Store invoice information
    - `invoice_items` - Store individual tasks included in invoices

  2. New Enums
    - `receivable_status` - Track receivable payment status
    - `invoice_status` - Track invoice lifecycle
    
  3. New Columns
    - Add `hours_worked` and `estimated_hours` to tasks
    - Add `invoice_status` to tasks
    - Add `hourly_rate`, `fixed_rate`, and `rate_type` to projects
*/
